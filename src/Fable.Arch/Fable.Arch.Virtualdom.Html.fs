...
open Fable.Arch.Virtualdom.Bindings
...
module Types =
    ...
    | Element of Element<'TMessage> * DomNode<'TMessage> []
    ...
    | Svg of Element<'TMessage> * DomNode<'TMessage> []

    let toAttrs handler (attrs:Attribute<'T> list) =
...
let rec map<'T1,'T2> (mapping:('T1 -> 'T2)) (node:DomNode<'T1>) = 
    match node with
    | Element(e,ns) -> Element(mapElem mapping e, ns |> Array.map (map mapping))
    | VoidElement(ve) -> VoidElement(mapVoidElem mapping ve)
    | Text(s) -> Text s 
    | WhiteSpace(ws) -> WhiteSpace ws   
    | Svg(e,ns) -> Element(mapElem mapping e, ns |> Array.map (map mapping))

[<AutoOpen>]
module Tags =
    let inline elem tagName attrs children = new VNode(tagName, (toAttrs (fun _ -> ()) attrs), children |> List.toArray) |> VNode// Element((tagName, attrs), children)
    let voidElem tagName attrs = elem tagName attrs [] // VoidElement(tagName, attrs)

    let inline text x : VDomNode = new VText(x) |> VText
    let whiteSpace = text
