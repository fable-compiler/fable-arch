...
function update(model, action) {
  if (action.Case === "ServerResponse") {
    return [function () {
      var Status_1 = new Status("Done", []);
      return new Model(model.InputValue, action.Fields[0], Status_1);
    }(), new List$1()];
  } else if (action.Case === "SetStatus") {
    return [new Model(model.InputValue, model.ServerResponse, action.Fields[0]), new List$1()];
  } else if (action.Case === "SendEcho") {
    var message = ofArray([function (h$$1) {
      fakeAjax(function (data) {
        return h$$1(new Actions("ServerResponse", [data]));
      }, model.InputValue);
      return h$$1(new Actions("SetStatus", [new Status("Pending", [])]));
    }]);
    return [model, message];
  } else {
    return [new Model(action.Fields[0], model.ServerResponse, model.Status), new List$1()];
  }
}
function view(model) {
  var resultArea = model.Status.Case === "Pending" ? Tags.span(new List$1())(ofArray([Tags.text("Waiting Server response")])) : model.Status.Case === "Done" ? Tags.span(new List$1())(ofArray([Tags.text("The server response is:"), Tags.br(new List$1()), Tags.b(new List$1())(ofArray([Tags.text(model.ServerResponse)]))])) : Tags.span(new List$1())(ofArray([Tags.text("")]));
  return Tags.div(ofArray([Attributes.classy("columns is-flex-mobile")]))(ofArray([Tags.div(ofArray([Attributes.classy("column")]))(new List$1()), Tags.div(ofArray([Attributes.classy("column is-narrow is-narrow-mobile"), new Types.Attribute("Style", [ofArray([["width", "400px"]])])]))(ofArray([Tags.label(ofArray([Attributes.classy("label"), Attributes.property("for", "input-area")]))(ofArray([Tags.text("Enter a sentence: ")])), Tags.p(ofArray([Attributes.classy("control")]))(ofArray([Tags.textarea(ofArray([Attributes.property("id", "input-area"), Attributes.classy("textarea"), Events.onEvent("oninput", function (e) {
    return function (arg0) {
      return new Actions("ChangeInput", [arg0]);
    }(e.target.value);
  }), Attributes.property("value", model.InputValue)]))(new List$1())])), Tags.div(ofArray([Attributes.classy("control")]))(ofArray([Tags.p(new List$1())(ofArray([Tags.button(ofArray([Attributes.classy("button"), Events.onMouseClick(function (_arg1) {
    return new Actions("SendEcho", []);
  })]))(ofArray([Tags.text("Uppercase by server")]))]))])), resultArea])), Tags.div(ofArray([Attributes.classy("column")]))(new List$1())]));
}
